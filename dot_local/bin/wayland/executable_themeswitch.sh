...
if [ ! -z "$(grep '^1|' "$ThemeCtl" | awk -F '|' '{print $3}')" ] ; then
  ...
  # nvim
    ...
    vimColor=$(grep '^1|' "$ThemeCtl" | awk -F '|' '{print $3}' | cut -d '~' -f 3)
    sed  -i "s/colorscheme = \"\([^\"]*\)\"/colorscheme = \"${vimColor}\"/" $HOME/.config/nvim/lua/config/init.lua 
fi
